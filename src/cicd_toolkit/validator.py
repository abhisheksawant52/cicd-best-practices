"""Load GitHub Actions workflows and evaluate them against best-practice rules."""
...
def evaluate(paths: Iterable[Path]) -> list[Finding]:
    ...
    for path in paths:
        ...
        for rule in ALL_RULES:
            findings.extend(rule(workflow.document, str(workflow.path)))
    return findings
...
def exceeds_threshold(findings: Iterable[Finding], fail_on: str) -> bool:
    ...
    threshold = _SEVERITY_ORDER.get(fail_on, 2)
